  ...
  Future<void> getStoreInfo() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (myMap['data'] != '' && myMap['store'] != '') {
        parser.saveStoreInfo(myMap['store']['id'], myMap['store']['name'], myMap['store']['cover'], int.parse(myMap['store']['cid'].toString()), myMap['store']['address'], myMap['store']['zipcode'], myMap['store']['mobile']);
      } else {
        showToast('Something went wrong');
      }
      update();
