...
class CheckoutController extends GetxController implements GetxService {
  ...
  String selectedTime = 'Today - '.tr + Jiffy.now().yMMMMd;
  String selectedDeliveryTime = Jiffy.now().format(pattern: 'yyyy-MM-dd hh:mm:ss');
  ...
  void saveDeliveryTime(String deliveryTime, String time) {
    if (time == 'today') {
      selectedDeliveryTime = Jiffy.now().format(pattern: 'yyyy-MM-dd hh:mm:ss');
    } else {
      selectedDeliveryTime = Jiffy.now().add(days: 1).format(pattern: 'yyyy-MM-dd hh:mm:ss');
    }
  ...
  Future<void> getCoupons(context) async {
    ...
    if (response.statusCode == 200) {
      ...
      body.forEach((data) {
        ...
        datas.dateTime = Jiffy.parse(datas.dateTime.toString()).yMMMMd;
  ...
  Future<void> createOrder() async {
    ...
    _orderNotes = [];
    var orderNotesParam = {'status': 1, 'value': 'Order Created', 'time': Jiffy.now().format(pattern: 'MMMM do yyyy, h:mm:ss a')};
