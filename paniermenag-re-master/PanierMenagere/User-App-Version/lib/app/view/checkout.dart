...
class _CheckoutScreenState extends State<CheckoutScreen> {
                        ...
                        child: Row(
                          ...
                          children: [
                            value.currencySide == 'left'
                                ? Text('${value.totalItems}${' Items : '.tr}${value.currencySymbol} ${value.grandTotal}', style: const TextStyle(fontSize: 14, fontFamily: 'medium', color: ThemeProvider.whiteColor))
                                : Text('${value.totalItems}${' Items : '.tr}${value.grandTotal} ${value.currencySymbol}', style: const TextStyle(fontSize: 14, fontFamily: 'medium', color: ThemeProvider.whiteColor)),
                            TextButton(
                              onPressed: () {
                                if (value.deliveryOrderTo == 'home' && _currentIndex == 0) {
                                  value.getMyAddressList();
  ...
  _showPopupMenu(Offset offset, value) async {
    ...
    await showMenu(
      ...
      items: [
        for (var item in AppConstants.deliveryTimes) PopupMenuItem<String>(value: item.id.toString(), onTap: () => value.saveDeliveryTime(item.name.toString(), item.id.toString()), child: Text(item.name)),
      ],
      elevation: 8.0,
    );
  }
    ...
    return Column(
      children: [
          ...
          children: [
            for (var options in AppConstants.deliveryOptions)
              ListTile(
                ...
                trailing: Radio(fillColor: WidgetStateProperty.resolveWith(getColor), value: options.id, groupValue: value.deliveryOrderTo, onChanged: (e) => value.saveDeliveryOrder(e)),
              )
          ],
        ),
        ...
        Container(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: <Widget>[Text('Taxes & Charges'.tr), value.currencySide == 'left' ? Text(value.currencySymbol + value.orderTax.toString()) : Text(value.orderTax.toString() + value.currencySymbol)],
          ),
        ),
        value.discount > 0
            ? Container(
