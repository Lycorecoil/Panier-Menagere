...
class StoresModel {
  ...
  StoresModel.fromJson(Map<String, dynamic> json) {
    ...
    if (json['open_time'] != null && json['open_time'] != '') {
      openTime = Jiffy.parse('2011-10-31 ${json['open_time'].toString()}').format(pattern: "hh:mm a");
    } else {
      openTime = '08:00 AM';
    }
    if (json['close_time'] != null && json['close_time'] != '') {
      closeTime = Jiffy.parse('2011-10-31 ${json['close_time'].toString()}').format(pattern: "hh:mm a");
    } else {
      closeTime = '10:00 PM';
    }
