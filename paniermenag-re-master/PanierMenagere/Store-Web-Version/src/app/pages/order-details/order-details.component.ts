...
export class OrderDetailsComponent implements OnInit {
      ...
      if (data && data.status && data.status == 200 && data.data) {
        ...
        this.orders.forEach((element) => {
          ...
          if (element && element.discount == 0) {
            if (element.size == 1) {
              ...
              price = price + (parseFloat(element.original_price) * element.quantity);
            }
          } else {
            if (element.size == 1) {
              if (element.variations[0].items[element.variant].discount && element.variations[0].items[element.variant].discount !== 0) {
        ...
        if (((x) => { try { JSON.parse(x); return true; } catch (e) { return false } })(info.extra)) {
          ...
          if (extras && extras.length) {
            ...
            if (extras && storeExtra.length && info.order_to == 'home' && info.delivery_charge > 0) {
  ...
  getTotalBilling() {
    ...
    const returnValue = total - discount > 0 ? total - discount : 0;
    return roundToDecimalPlace(returnValue, 2);
  }
