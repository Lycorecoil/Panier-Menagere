  ...
  getStasData() {
    this.api.post_private('v1/orders/getStoreStatsData', { id: localStorage.getItem('uid') }).then((data: any) => {
      ...
      if (data && data.status && data.status == 200 && data.data) {
        ...
        let allOrders: any[] = [];
        function roundToDecimalPlace(number: any, decimalPlaces: any) {
                    ...
                    if (order && order.discount == 0) {
                      if (order.size == 1) {
                        if (order.variations[0].items[order.variant].discount && order.variations[0].items[order.variant].discount !== 0) {
                          price = price + (parseFloat(order.variations[0].items[order.variant].discount) * order.quantity);
                        } else {
                          price = price + (parseFloat(order.variations[0].items[order.variant].price) * order.quantity);
                        }
                      } else {
                        price = price + (parseFloat(order.original_price) * order.quantity);
                      }
                    } else {
                      if (order.size == 1) {
                        if (order.variations[0].items[order.variant].discount && order.variations[0].items[order.variant].discount !== 0) {
                        ...
                        price = price + (parseFloat(order.sell_price) * order.quantity);
                      }
                    }
